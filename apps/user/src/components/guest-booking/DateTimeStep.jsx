...
import { ChevronLeft, ChevronRight, RefreshCw, Lock, Calendar as CalendarIcon, Clock as ClockIcon, Info, ArrowRight, MousePointer2 } from 'lucide-react';
...
const DateTimeStep = ({
    ...
    const formatHoldDateTime = () => {
        ...
        return `${dayName} ${monthNames[date.getMonth()]} ${date.getDate()}, ${date.getFullYear()} at ${activeHold.time}`;
    };
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className='grid grid-cols-1 lg:grid-cols-[1fr_0.8fr] gap-8 mb-10'>
                ...
                <div className='bg-white dark:bg-white/[0.02] border border-gray-100 dark:border-gray-800 rounded-3xl p-5 shadow-theme-sm h-fit'>
                    <div className='flex items-center justify-between mb-5'>
                        <h3 className='text-[15px] sm:text-base font-bold text-gray-900 dark:text-white flex items-center gap-2 tracking-tight uppercase'>
                        ...
                        <div className='flex gap-1.5'>
                            <button
                                ...
                                disabled={!canGoPrev}
                                className='p-2 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl hover:bg-white dark:hover:bg-gray-700 transition-all disabled:opacity-30 hover:shadow-theme-xs'
                            >
                    ...
                    <div className='grid grid-cols-7 gap-1 mb-2'>
                        {dayNames.map(day => (
                            <div key={day} className='text-[10px] sm:text-[11px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest text-center py-2'>
                                {day}
                            </div>
                        ))}
                    </div>
                    ...
                    <div className='grid grid-cols-7 gap-1.5'>
                        {calendarDays.map((date, idx) => {
                            ...
                            const isDisabled = isPast || isToday || isSunday || isBeyondMax;
                            ...
                            return (
                                <button
                                    ...
                                    disabled={isDisabled}
                                    className={`
                                        relative flex flex-col items-center justify-center aspect-square rounded-xl transition-all duration-300
                                        ${isSelected 
                                            ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/30 scale-105 z-10' 
                                            : isDisabled 
                                                ? 'text-gray-300 dark:text-gray-700 cursor-not-allowed opacity-30 shadow-none bg-transparent' 
                                                : 'bg-gray-50/50 dark:bg-gray-800/30 hover:bg-white dark:hover:bg-gray-800 border-2 border-transparent hover:border-brand-200 dark:hover:border-brand-500/50 text-gray-700 dark:text-gray-300 shadow-theme-xs'
                                        }
                                    `}
                                >
                                    ...
                                    {isToday && !isSelected && (
                                        <div className="absolute bottom-1 sm:bottom-1.5 w-1 h-1 rounded-full bg-brand-500" />
                                    )}
                                </button>
                            );
                        })}
                ...
                <div className='flex flex-col min-h-[380px]'>
                    {!selectedDate ? (
                        ...
                        <div className='flex-grow bg-gray-50 dark:bg-white/[0.02] border border-dashed border-gray-200 dark:border-gray-800 rounded-3xl flex flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in duration-500'>
                            <div className='bg-white dark:bg-gray-800 w-14 h-14 rounded-2xl flex items-center justify-center shadow-theme-sm mb-5'>
                                <MousePointer2 size={28} className='text-brand-500' />
                            </div>
                            <h4 className='text-base font-bold text-gray-900 dark:text-white mb-1.5 tracking-tight uppercase'>
                                Pick a Date
                            </h4>
                            <p className='text-[12px] text-gray-500 dark:text-gray-400 max-w-[220px] leading-relaxed font-bold'>
                                Select an available day from the calendar to see slots.
                            </p>
                        </div>
                    ) : (
                        /* TIME SELECTION Area */
                        <div className='animate-in fade-in slide-in-from-right-4 duration-500 h-full flex flex-col'>
                            ...
                            {/* Reserved Slot Indicator */}
                            {activeHold && selectedDate === activeHold.date && (
                                <div className='mb-5 p-4 bg-brand-50/50 dark:bg-brand-500/10 border border-brand-100 dark:border-brand-500/20 rounded-2xl'>
                                    <div className='flex items-start gap-4'>
                                        ...
                                        <div className='grow'>
                                            <p className='text-[11px] font-black text-gray-900 dark:text-white uppercase tracking-tight'>Slot Reserved</p>
                                            <p className='text-[12px] text-brand-600 dark:text-brand-400 font-bold mt-0.5 leading-tight'>
                                                {formatHoldDateTime()}
                                            </p>
                                            <div className='flex items-center gap-2 mt-2'>
                                                ...
                                                <span className='text-[9px] font-black text-gray-500 whitespace-nowrap uppercase'>
                                                    {formattedTime}
                                                </span>
                                            </div>
                                        </div>
                            ...
                            <div className='flex items-center justify-between mb-5'>
                                ...
                                <button
                                    ...
                                    className='flex items-center gap-2 px-3 py-1.5 text-[11px] font-bold bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-400 rounded-lg border border-gray-100 dark:border-gray-700 shadow-theme-xs transition-all disabled:opacity-50'
                                >
                            ...
                            {slotsLoading ? (
                                ...
                                <div className='grid grid-cols-2 xsm:grid-cols-3 gap-3'>
                                    {slots.map((slot) => {
                                        ...
                                        const effectiveAvailable = slot.available;
                                        if (effectiveAvailable <= 0 && !isHeldByMe) return null;
                                        ...
                                        return (
                                            <button
                                                ...
                                                title={`${effectiveAvailable} slot${effectiveAvailable !== 1 ? 's' : ''} available`}
                                                className={`py-3 rounded-xl text-[12px] font-bold transition-all relative ${
                                                    isSelected
                                ...
                                <div className='p-8 bg-gray-50 dark:bg-white/[0.02] rounded-2xl text-center border-2 border-dashed border-gray-200 dark:border-gray-800 flex-grow flex flex-col items-center justify-center leading-relaxed'>
                                    <p className='text-gray-500 text-[14px] font-bold mb-2'>No available slots for this date.</p>
                                    {nextAvailableDate && (
                                        <button
                                            onClick={() => {
                                            ...
                                            className='text-brand-500 text-[13px] font-black hover:underline underline-offset-4'
                                        >
            ...
            <div className='flex justify-between items-center pt-8 border-t border-gray-100 dark:border-gray-700'>
                <button
                    onClick={onBack}
                    className='text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white font-black text-[14px] sm:text-base px-6 py-3 transition-colors'
                >
                    Back
                </button>
                <button
                    ...
                    className='bg-brand-500 hover:bg-brand-600 active:scale-95 text-white font-black px-10 py-4 rounded-2xl transition-all shadow-theme-md disabled:opacity-30 disabled:cursor-not-allowed flex items-center gap-2.5 text-[14px] sm:text-base'
                >
                    Review Details
                    <ArrowRight size={20} />
                </button>
